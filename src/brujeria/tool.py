...
from .appdirs import CACHE_HOME
...
def argument (options, var, value):
    ...
    opts.append('-D{}={}'.format(var, value))
...
class CMake:
    def __init__(self, spec, state):
        ...
        self.package = ''.join(name[:-1]) # Turns into joined path, becomes 'install' root
        self.name = name[-1]
    ...
    def configure (self):
        if os.path.exists(self.binary_dir): return
        options = [
            CMAKE_PRG,
            '-G',
            'Ninja',
            '-H{}'.format(os.fspath(self.source_dir)),
    ...
    def target (self, target):
        options = [
            CMAKE_PRG,
            '--build',
            os.fspath(self.binary_dir),
            '--target',
            target,
        ]
        run(options).check_returncode()

    def install (self): self.target('install')
    def build (self): self.target('all')
